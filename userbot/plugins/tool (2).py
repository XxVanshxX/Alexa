...
import asyncio, time, io, math, os, logging, asyncio, shutil, re, subprocess
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        await rkp.edit("Error usage tts <langcode> <reply to a media>")
        return
    text = text.strip()
    lan = lan.strip()
    if not os.path.isdir(TEMP_DOWNLOAD_DIRECTORY):
...
\n\n`!tts <text> (or reply) **or** `!tts2` <langcode> <reply to a media>`\
