...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "No name set yet nibba, check pinned in @"
FILLED_UP_DADDY = "Invalid pack selected."

@borg.on(admin_cmd(pattern="m ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    ...
    packname = f"{user.first_name}'.{pack}"
